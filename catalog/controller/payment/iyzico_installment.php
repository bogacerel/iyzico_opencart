...
class ControllerPaymentIyzicoInstallment extends Controller {
    ...
    protected function index() {        
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/iyzico_installment.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/iyzico_installment.tpl';
        } else {
            $this->template = 'default/template/payment/iyzico_installment.tpl';
        }
        $this->load->model('checkout/order');
        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);              
        if ($this->config->get('iyzico_installment_test')) {
            $merchantApiId = $this->config->get('iyzico_installment_api_id_test');
            $merchantSecretKey = $this->config->get('iyzico_installment_secret_key_test');          
        } else {
            $merchantApiId = $this->config->get('iyzico_installment_api_id_live');
            $merchantSecretKey = $this->config->get('iyzico_installment_secret_key_live');
        }
        $identificationTransId = uniqid("opencart_MBCR");
        $this->data['mode'] = $this->config->get('iyzico_installment_installment_test');
        $paymentType = 'CC';
        ...
        if (function_exists('curl_version')) {
            ...
            $this->load->model('setting/iyzico_installment');            
            $this->data['payonJsPath'] = $this->model_setting_iyzico_installment->getPayonJSURL($this->transactionMode,true);            
            #get site url.
            $this->data['siteUrl'] = $this->model_setting_iyzico_installment->getSiteUrl();
    ...
    private function __generateToken($merchantApiId, $merchantSecretKey, $identificationTransId, $paymentType, $presAmount, $presCurrency, $presUsage, $order_info) {
        ...
        $this->postFieldsArray = array(
            ...
            'amount' => $this->model_setting_iyzico_installment->amountConverter($presAmount),
            'currency' => urlencode($presCurrency),
            'return_url' =>  $this->model_setting_iyzico_installment->getSiteUrl().'index.php?route=payment/iyzico_installment/send',
        ...
        $response = $this->model_setting_iyzico_installment->curlCall($this->postFieldsArray,'tokenGeneration'); 
        return $response;
    }
    ...
    public function send() {               
        ...
        $json = array();
        $respMsg = '';
        if (isset($jsonResp['transaction']['processing']['result']) || isset($jsonResp['response']['state'])) {
            if (isset($jsonResp['transaction']['processing']['result'])) {
                if ($jsonResp['transaction']['processing']['result'] == "ACK") {
                    $message = '';
                    ...
                    #update total amount for installment payment.            
                    $newAmount = str_replace(',', '', $jsonResp['transaction']['payment']['installment']['final_amount']);   
                    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET total = '" .$newAmount . "' WHERE order_id = '" . (int)$orderId . "'");                        
                    $this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('iyzico_installment_order_status_id'), $message, false);            
                    #for installment payment "brandname : 9 - Taksit" need to add new entry
                    $brand = $jsonResp['transaction']['account']['brand'];
                    $numberOfInstallments = $jsonResp['transaction']['payment']['installment']['number'];
                    $comment = $brand.' - '. $numberOfInstallments .'  Taksit';
                    $this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$orderId . "', order_status_id = '" . $this->config->get('iyzico_installment_order_status_id') . "', notify = '0', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");          
                    ...
                    $url = $this->url->link('payment/iyzico_installment/error');
                    $this->redirect($url);
                }
            } else if ($jsonResp['response']['state'] == "failed") {
                $json['error'] = $jsonResp['response']['error_message'];
                $this->session->data['error'] = $jsonResp;
                $url = $this->url->link('payment/iyzico_installment/error');
                $this->redirect($url);
            }
